...
class DetalhesMedicacao : ComponentActivity() {
...
@Composable
fun DetalhesMedicacaoScreen(medViewModel: MedViewModel = viewModel()) {
    val medId = (LocalContext.current as ComponentActivity).intent.getStringExtra("id_medicacao")
    val medData by medViewModel.medicacao.observeAsState()
    val context = LocalContext.current

    LaunchedEffect(medId) {
        medId?.let {
            medViewModel.loadMedicacaoById(it)
        } ?: Toast.makeText(context, "Erro ao carregar pet", Toast.LENGTH_SHORT).show()
    }

// Estabelecendo os estados para os campos
    ...
    LaunchedEffect(medData) {
        medData?.let { med ->
            //carregando os dados da medicação no estado
            nomeState = TextFieldValue(med.nome)
            tipoState = med.tipo

            //conversão de timestamp para formato legível
            dataState = med.data.let { timestamp ->
                try {
                    val date = timestamp.toDate() //converte o timestamp para date
                    SimpleDateFormat("dd/MM/yyyy", Locale.getDefault()).format(date) //formata a data
                } catch (e: Exception) {
                    ""
                }
            } ?: "" //retorna string vazia

            doseReforcoState = med.dose_reforco?.let { timestamp ->
                try {
            ...
            obsState = TextFieldValue(med.observacoes)
            lembreteState = med.lembrete
        }
    }
    ...
    Column(
        ...
        CustomButton(
            onClickAction = {
                medId?.let {
                    MedRepository.updateMedicamento(
                        idMedicacao = it,
                        nome = nomeState.text,
                        tipo = tipoState,
